...
public class ConcentrationBoard {
    ...
    public CardMatch reveal(int row, int col) throws ConcentrationException {
        //TODO YOUR CODE HERE
        if (gameOver()) throw new ConcentrationException("Game already over");
        if (this.revealedCard == null) {
            ConcentrationCard card = getCard(row, col);
            if (!card.isHidden()) {
                throw new ConcentrationException("Card is revealed");
            }
            this.revealedCard = card;
            card.reveal();
            return new CardMatch(card, null, false);
        } else {
            ConcentrationCard card1 = this.revealedCard;
            ConcentrationCard card2 = getCard(row, col);
            if (card1 == card2 || !card2.isHidden()) {
                throw new ConcentrationException("Card is revealed");
            }
            boolean match = card1.getLetter()==card2.getLetter();
            if(match){
                card1.reveal();
                card2.reveal();
            } else {
            ...
            this.revealedCard = null;
            return new CardMatch(card1, card2, match);
        }
    }
